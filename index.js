...
function menu(){
    ...
    .addEventListener("mouseover",function(dets){
        ...
        cur.style.top = `${dets.clientY-25}px`
       cur.style.left = `${dets.clientX-25}px`
    })
    document.querySelector("#hamburger")
...
document.querySelector(".overlay>h1").textContent.split("").forEach(function (elem) {
    clutter +=
       `<span>${elem}</span>`
});
...
document.querySelector(".overlay>p").textContent.split("").forEach(function (elem) {
    clutter +=
       `<span>${elem}</span>`
});
document.querySelector(".overlay>p").innerHTML = clutter;
...
gsap.to(".overlay p span", {
    opacity: 0,
    stagger:.8,
    scrollTrigger: {

        Scroller: "#home",
        trigger: ".overlay p span",
        start: "bottom 20%",
        // markers:true,
        scrub: 1,
    }
